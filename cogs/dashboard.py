...
class Dashboard(commands.Cog):
    ...
    async def owner_panel(self, request):
        ...
        <body class="bg-[#09090b] text-zinc-300 font-sans min-h-screen flex flex-col selection:bg-red-500 selection:text-white">
            ...
            <main class="flex-1 max-w-5xl w-full mx-auto p-6 lg:p-8 flex flex-col gap-8">
                ...
                <div class="glass-panel rounded-2xl p-6 border border-red-500/30 bg-red-500/5 relative overflow-hidden">
                    <h2 class="text-xl font-bold text-white mb-4">Loaded Core Extensions</h2>
                    <p class="text-red-300 font-mono text-sm mb-6 bg-black/40 p-4 rounded-xl border border-red-500/20">{loaded_cogs}</p>
                    
                    <p class="text-xs text-zinc-500"><i class="fa-solid fa-info-circle"></i> Note: For live hot-reloading or global broadcast commands, please use the registered /owner commands directly within Discord.</p>
                </div>
            </main>
        </body>
        </html>
    ...
    async def manage_server(self, request):
        ...
        settings_data = {
            ...
            "banned_words": ["unauthorized_term_1", "prohibited_phrase"]
        }
        ...
        ai_checked = "checked" if settings_data["ai_enabled"] else ""
        mod_checked = "checked" if settings_data["mod_enabled"] else ""
        anime_checked = "checked" if settings_data["anime_enabled"] else ""
        sports_checked = "checked" if settings_data["sports_enabled"] else ""
        misc_checked = "checked" if settings_data["misc_enabled"] else ""
        
        nexusify_checked = "checked" if settings_data["default_ai_model"] == "nexusify" else ""
        gemini_checked = "checked" if settings_data["default_ai_model"] == "gemini" else ""
        sarvam_checked = "checked" if settings_data["default_ai_model"] == "sarvam" else ""
        banned_words_str = ", ".join(settings_data["banned_words"])
        ...
        <body class="bg-[#09090b] text-zinc-300 font-sans min-h-screen flex flex-col selection:bg-violet-500 selection:text-white relative">
            ...
            <div id="modModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
                <div class="glass-panel w-full max-w-lg rounded-2xl p-6 border border-emerald-500/30 shadow-2xl">
                    ...
                    <div class="mb-6">
                        <textarea id="banned_words_input" rows="4" class="w-full bg-[#18181b] border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-emerald-500 transition resize-none placeholder-zinc-600" placeholder="e.g. badword, spamlink">__BANNED_WORDS__</textarea>
                    </div>
                    <div class="flex justify-end gap-3">
        ...
        manage_html = manage_html.replace("__GUILD_NAME__", str(guild.name)).replace("__GUILD_ICON__", str(guild_icon)).replace("__BOT_NAME__", str(bot_name)).replace("__USER_NAME__", str(user_name)).replace("__USER_AVATAR__", str(user_avatar)).replace("__GUILD_ID__", str(guild_id_int))
        manage_html = manage_html.replace("__AI_CHECKED__", ai_checked).replace("__MOD_CHECKED__", mod_checked).replace("__ANIME_CHECKED__", anime_checked).replace("__SPORTS_CHECKED__", sports_checked).replace("__MISC_CHECKED__", misc_checked)
        manage_html = manage_html.replace("__NEXUSIFY_CHECKED__", nexusify_checked).replace("__GEMINI_CHECKED__", gemini_checked).replace("__SARVAM_CHECKED__", sarvam_checked).replace("__BANNED_WORDS__", banned_words_str)
    ...
    async def update_settings(self, request):
        ...
        guild_id_int = int(request.match_info.get('guild_id', 0))
        guild = self.bot.get_guild(guild_id_int)
        if not guild: return web.json_response({"error": "Not in server"}, status=404)
            
        member = guild.get_member(int(user_session['discord_id']))
        if not member or not (member.guild_permissions.administrator or member.guild_permissions.manage_guild): return web.json_response({"error": "Forbidden"}, status=403)
        ...
        try:
            ...
            if action == 'update_ai_model':
                if hasattr(self.bot, 'db'): await self.bot.db.guild_settings.update_one({"guild_id": guild_id_int}, {"$set": {"default_ai_model": data.get('model')}}, upsert=True)
                return web.json_response({"success": True})
            elif action == 'update_automod':
                words_list = [w.strip().lower() for w in data.get('words', '').split(',') if w.strip()]
                if hasattr(self.bot, 'db'): await self.bot.db.guild_settings.update_one({"guild_id": guild_id_int}, {"$set": {"banned_words": words_list}}, upsert=True)
                return web.json_response({"success": True})
            elif action == 'toggle':
                module = data.get('module')
                db_mapping = {'toggleAI': 'ai_enabled', 'toggleMod': 'mod_enabled', 'toggleAnime': 'anime_enabled', 'toggleSports': 'sports_enabled', 'toggleMisc': 'misc_enabled'}
                if module not in db_mapping: return web.json_response({"error": "Invalid module name"}, status=400)
                if hasattr(self.bot, 'db'): await self.bot.db.guild_settings.update_one({"guild_id": guild_id_int}, {"$set": {db_mapping[module]: data.get('enabled')}}, upsert=True)
                return web.json_response({"success": True})
    ...
    async def login(self, request):
        ...
        if not client_id or not redirect_uri: return web.Response(text="Configuration Error: Env vars missing.", status=500)
        raise web.HTTPFound(f"https://discord.com/api/oauth2/authorize?client_id={client_id}&redirect_uri={urllib.parse.quote(redirect_uri)}&response_type=code&scope=identify%20guilds")

    async def callback(self, request):
        code = request.query.get("code")
        if not code: return web.Response(text="Login failed. No code provided by Discord.", status=400)

        data = {"client_id": os.getenv("DISCORD_CLIENT_ID"), "client_secret": os.getenv("DISCORD_CLIENT_SECRET"), "grant_type": "authorization_code", "code": code, "redirect_uri": os.getenv("REDIRECT_URI")}
        
        async with aiohttp.ClientSession() as session:
            async with session.post("https://discord.com/api/oauth2/token", data=data, headers={"Content-Type": "application/x-www-form-urlencoded"}) as resp:
                if resp.status != 200: return web.Response(text="Auth Failed", status=500)
        ...
        if hasattr(self.bot, 'db'):
            await self.bot.db.sessions.update_one({"discord_id": user_data["id"]}, {"$set": {"session_id": session_id, "username": user_data.get("username", "Unknown"), "avatar": user_data.get("avatar", ""), "access_token": access_token, "created_at": datetime.datetime.utcnow().timestamp()}}, upsert=True)
    ...
    async def logout(self, request):
        session_id = request.cookies.get("recluse_session")
        if session_id and hasattr(self.bot, 'db'): await self.bot.db.sessions.delete_one({"session_id": session_id})
        response = web.HTTPFound('/')
    ...
    async def cog_unload(self):
        if self.runner: self.bot.loop.create_task(self.runner.cleanup())
