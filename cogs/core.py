...
class Core(commands.Cog):
    ...
    async def custom_help(self, ctx):
        embed = discord.Embed(
            ...
            description="Please select a category from the dropdown menu below to view available commands.", 
    ...
    async def botinfo(self, ctx):
        ...
        # Access user_ai_preference from the AI cog if it exists, otherwise default
        ai_cog = self.bot.get_cog('AI')
        active_ai = "Nexusify"
        if ai_cog:
            active_ai = ai_cog.user_ai_preference.get(ctx.author.id, "nexusify").title()
        
        app_info = await self.bot.application_info()
        
        embed = discord.Embed(title="System Telemetry & Application Information", color=discord.Color.blue())
    ...
    @commands.hybrid_command(name="clear_cooldown", description="[Dev] Wipes your active cooldown for a specific command.")
    @commands.is_owner()
    async def clear_cooldown(self, ctx, command_name: str):
        cmd = self.bot.get_command(command_name)
        if not cmd:
            return await ctx.send(f"❌ I couldn't find a command named `{command_name}`.", ephemeral=True)
        cmd.reset_cooldown(ctx)
        await ctx.send(f"✅ Successfully wiped your cooldown for `/{command_name}`. Fire away!", ephemeral=True)
    ...
    async def cycle_bot_status(self):
        try:
            ...
            if active_matches > 0:
                match_text = "match" if active_matches == 1 else "matches"
                current_status = f"{active_matches} live cricket {match_text}"
                ...
                raw_status = next(self.status_cycle)
                current_status = raw_status.replace("{servers}", str(server_count)).replace("{members}", str(member_count))
    ...
    async def auto_broadcast(self):
        channel = self.bot.get_channel(1096869180621463662) 
        if channel:
            await channel.send("Automated Autonomous Broadcast: All structural system operations remain nominal.")
