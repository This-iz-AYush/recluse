...
"""
import asyncio, datetime, random, re
...
class Fun(commands.Cog):
    ...
    async def eightball(self, i:discord.Interaction, question:str):
        ...
        e.add_field(name="Answer",value=random.choice(_8BALL),inline=False)
    ...
    async def fact(self, i:discord.Interaction):
        await i.response.send_message(embed=discord.Embed(title="💡 Fact",description=random.choice(_FACTS),color=discord.Color(0x1ABC9C)))
    ...
    async def quote(self, i:discord.Interaction):
        ...
        await i.response.send_message(embed=discord.Embed(description=f'*"{q}"*\n\n— **{a}**',color=discord.Color(0x3498DB)))
    ...
    async def compliment(self, i:discord.Interaction, member:discord.Member=None):
        t=member or i.user
        e=discord.Embed(title=f"💖 Compliment for {t.display_name}",description=random.choice(_COMPLIMENTS),color=discord.Color(0xFF69B4))
        await i.response.send_message(embed=e)
    ...
    async def roast(self, i:discord.Interaction, member:discord.Member):
        r=random.choice(_ROASTS)
        e=discord.Embed(title=f"🔥 Roast: {member.display_name}",description=r,color=discord.Color(0xFF4500))
    ...
    async def rps(self, i:discord.Interaction, choice:app_commands.Choice[str]):
        ...
        u=choice.value; wins={"rock":"scissors","paper":"rock","scissors":"paper"}
    ...
    async def trivia(self, i:discord.Interaction):
        ...
        _ACTIVE_TRIVIA[i.channel.id]=str(q["correct"])
        e=discord.Embed(title="🧠 Trivia!",description=q["q"],color=discord.Color(0x2ECC71))
        opts="\n".join(f"{'ABCD'[idx]}. {c}" for idx,c in enumerate(q["choices"]))
        ...
        try:
            ...
            if idx==q["correct"]:
                re_e=discord.Embed(title="✅ Correct!",description=f"{msg.author.mention} got it!\n**{q['choices'][q['correct']]}**",color=discord.Color.green())
            else:
                re_e=discord.Embed(title="❌ Wrong!",description=f"{msg.author.mention} said **{q['choices'][idx]}**\nAnswer: **{q['choices'][q['correct']]}**",color=discord.Color.red())
            await i.channel.send(embed=re_e)
        except asyncio.TimeoutError:
            await i.channel.send(embed=discord.Embed(title="⏰ Time's Up!",description=f"Answer: **{q['choices'][q['correct']]}**",color=discord.Color.orange()))
        finally:
